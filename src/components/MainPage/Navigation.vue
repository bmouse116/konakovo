    ...
    <div class="navigation">
        <div class="navigation-container">
            ...
            <div class="search-bar">
                <div class="search-input-wrapper" v-if="isMap">
                    <Input @click="openKeyboard" :handleVisible="handleVisible" :value="searchText" :infoOpen="infoOpen"
                        @input="updateSearchText" />
                    <!-- 2. Список подсказок находится ВНУТРИ обертки и никому не мешает -->
                    <ul v-show="searchResults.length > 0 && showSuggestions" class="search-suggestions">
                        <li v-for="point in searchResults" :key="point.id" @click="selectSuggestion(point)">
                            {{ point.title }}
                        </li>
                    </ul>
                </div>
            ...
            <Keyboard v-if="handleVisible" @close="closeKeyboard" @keypress="handleKeyPress"
...
const searchResults = computed(() => {
    // Не показываем подсказки, если введено мало символов или поле пустое
    if (!searchText.value || searchText.value.length < 2) {
        return [];
    }
    const query = searchText.value.toLowerCase();
    // Фильтруем только по списку, который пришел в props
    return props.searchablePoints.filter(point =>
        point.title.toLowerCase().includes(query)
    );
});

// ---> ДОБАВЬТЕ ЭТУ ФУНКЦИЮ, ЧТОБЫ ВЫБРАТЬ ПОДСКАЗКУ
const selectSuggestion = (point: Point) => {
    searchText.value = point.title;
    emit('search-update', searchText.value);
    showSuggestions.value = false; // <--- ГЛАВНОЕ ИЗМЕНЕНИЕ: Скрываем список
    closeKeyboard(); // <-- Приятный бонус: закрываем клавиатуру после выбора
};
...
const updateSearchText = (e: Event) => {
    ...
    if (target && target.value !== undefined) {
        ...
        showSuggestions.value = true; // <--- Показываем список снова при вводе
    } else {
        searchText.value = '';
        emit('search-update', '');
    }
...
const handleKeyPress = (key: string) => {
    ...
    emit('search-update', searchText.value);
    showSuggestions.value = true; // <--- И здесь тоже
};

const handleBackspace = () => {
    searchText.value = searchText.value.slice(0, -1);
    emit('search-update', searchText.value);
    showSuggestions.value = true; // <--- И здесь
};
